...
import {
  getWaiters, addWaiter, updateWaiter, deleteWaiter,
  toggleWaiter, resetWaiterPin, WaiterProfile,
} from '../services/api';
import { Colors, FontSize, Spacing, BorderRadius, Shadows } from '../utils/constants';

type Props = NativeStackScreenProps<RootStackParamList, 'WaiterManagement'>;
...
const WaiterManagementScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const openEdit = (w: WaiterProfile) => {
    setSelected(w);
    setName(w.name); setEmployeeCode(w.employeeCode); setMobile(w.mobile); setPin(''); setFormError('');
    setModalMode('edit');
  };

  const openPin = (w: WaiterProfile) => {
    setSelected(w);
    setPin(''); setFormError('');
    setModalMode('pin');
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (modalMode === 'add') {
        const w = await addWaiter({ name: name.trim(), employeeCode, pin, mobile });
        setWaiters(prev => [w, ...prev]);
      } else if (modalMode === 'edit' && selected) {
        const w = await updateWaiter(selected._id, { name: name.trim(), employeeCode, mobile });
        setWaiters(prev => prev.map(x => x._id === w._id ? w : x));
      }
      closeModal();
    } catch (e: any) {
  ...
  const handleResetPin = async () => {
    ...
    setSaving(true);
    try {
      if (selected) await resetWaiterPin(selected._id, pin);
      closeModal();
  ...
  const handleToggle = async (w: WaiterProfile) => {
    try {
      const { isActive } = await toggleWaiter(w._id);
      setWaiters(prev => prev.map(x => x._id === w._id ? { ...x, isActive } : x));
    } catch { /* silent */ }
  };

  const handleDelete = (w: WaiterProfile) => {
    Alert.alert('Delete Waiter', `Remove ${w.name} (${w.employeeCode})?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive', onPress: async () => {
          try {
            await deleteWaiter(w._id);
            setWaiters(prev => prev.filter(x => x._id !== w._id));
          } catch { /* silent */ }
        }
      },
    ]);
  ...
  const renderWaiter = ({ item }: { item: WaiterProfile }) => (
  ...
  return (
    <View style={[styles.container, { paddingTop: top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Manage Waiters</Text>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={styles.emptySub}>Tap Add to create the first waiter account</Text>
      ...
      <Modal visible={modalMode === 'add' || modalMode === 'edit'} transparent animationType="slide" onRequestClose={closeModal}>
        ...
        <View style={styles.sheet}>
          ...
          <Text style={styles.sheetTitle}>{modalMode === 'add' ? 'Add Waiter' : 'Edit Waiter'}</Text>
          ...
          <View style={styles.inputWrap}>
            ...
            <TextInput style={styles.input} placeholder="Employee Code (e.g. W001) *"
...
const styles = StyleSheet.create({
  ...
  waiterName:   { fontSize: FontSize.md, fontWeight: '800', color: Colors.text },
