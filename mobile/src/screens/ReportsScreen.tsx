...
<div class="footer">Generated by Dine POS &bull; ${new Date().toLocaleDateString('en-IN')}</div>
      ...
      const filename = `GST_Report_${gstFrom}_to_${gstTo}.html`;
      const fileUri = (FileSystem.documentDirectory || '') + filename;
      await FileSystem.writeAsStringAsync(fileUri, html, { encoding: FileSystem.EncodingType.UTF8 });
      await Sharing.shareAsync(fileUri, { mimeType: 'text/html', dialogTitle: 'Export GST Report' });
    } catch (e: any) {
...
<div class="footer">Generated by Dine POS &bull; ${new Date().toLocaleDateString('en-IN')}</div>
      ...
      const filename = `Tally_Export_${gstFrom}_to_${gstTo}.html`;
      const fileUri = (FileSystem.documentDirectory || '') + filename;
      await FileSystem.writeAsStringAsync(fileUri, html, { encoding: FileSystem.EncodingType.UTF8 });
      await Sharing.shareAsync(fileUri, { mimeType: 'text/html', dialogTitle: 'Tally Export' });
    } catch (e: any) {
