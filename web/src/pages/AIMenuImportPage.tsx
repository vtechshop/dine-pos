...
import {
  ...
  ExtractedProduct, ExtractedCategory, ImportProduct, ImportCategory, ImportResults,
} from '../api/aiMenu';
        ...
        <div className="mt-6 rounded-xl border border-border bg-canvas p-4">
          ...
          <div className="space-y-2">
            {[
              ...
              { icon: Sparkles,    text: 'Gemini reads and extracts all categories & products' },
...
function ReviewPanel({
  ...
  function addProduct(catId: string) {
    const blank: EditableProduct = {
      _id: uid(), name: '', price: null, variant: '', veg: true,
      gst: null, description: '', confidence: 1,
      isDuplicate: false, duplicateAction: 'skip',
      isDeleted: false, isEditing: true,
    };
    onUpdate(categories.map(c =>
  ...
  function addCategory() {
    onUpdate([...categories, {
      ...
      isDeleted: false, collapsed: false,
      products: [],
    }]);
  }
    ...
    <div className="overflow-hidden rounded-xl border border-border bg-white shadow-sm">
      ...
      <div className="flex items-center gap-2 border-b border-border bg-mist px-4 py-2.5">
        ...
        <button
          ...
          title="Add product"
          className="rounded-lg p-1 text-ink/30 hover:bg-brand/10 hover:text-brand transition-colors"
        >
          <Plus size={14} />
        </button>
        <button
        ...
        <button
          ...
          className="rounded-lg p-1 text-ink/30 hover:bg-red-50 hover:text-red-500 transition-colors"
        >
          <Trash2 size={13} />
        </button>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-[10px] font-semibold uppercase tracking-wide text-ink/30">
                ...
                <th className="px-3 py-2 text-left hidden md:table-cell">Variant</th>
      ...
      <tr className="bg-brand/[0.03]">
        ...
        <td className="px-3 py-2 hidden md:table-cell">
          <select
            ...
            className="rounded border border-border bg-white px-1 py-0.5 text-xs outline-none"
          >
            <option value="">—</option>
  ...
  return (
    <tr className={`group transition-colors ${prod.isDuplicate ? 'bg-amber-50' : 'hover:bg-mist'}`}>
      ...
      <td className="px-3 py-2.5 text-right tabular-nums">
        {prod.price !== null
          ? <span className="font-semibold text-ink">₹{prod.price}</span>
      ...
      <td className="px-3 py-2.5 hidden md:table-cell">
        {prod.variant
          ? <span className="rounded-full bg-border/60 px-2 py-0.5 text-[10px] font-semibold capitalize text-ink/50">{prod.variant}</span>
      ...
      <td className="px-3 py-2.5">
        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            ...
            className="rounded-lg p-1.5 text-ink/30 hover:bg-red-50 hover:text-red-500 transition-colors"
          >
            <Trash2 size={13} />
          </button>
        </div>
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-6 px-4">
      ...
      <div className="grid w-full max-w-sm grid-cols-2 gap-3">
        {[
          { label: 'Categories Created', value: results.categoriesCreated, color: 'text-brand' },
          { label: 'Products Created',   value: results.productsCreated,   color: 'text-green-600' },
...
export function AIMenuImportPage() {
  ...
  const handleFile = useCallback(async (f: File) => {
    ...
    try {
      ...
      setStep('checking');
      let dupeSet = new Set<string>();
      try {
        const { duplicates } = await checkDuplicates(result.categories);
        dupeSet = new Set(duplicates.map(d => `${d.categoryName}::${d.productName}`));
      } catch {
      ...
      const editable = buildEditable(result.categories).map(cat => ({
        ...
        products: cat.products.map(p => ({
          ...p,
          isDuplicate: dupeSet.has(`${cat.name}::${p.name}`),
        })),
      }));
